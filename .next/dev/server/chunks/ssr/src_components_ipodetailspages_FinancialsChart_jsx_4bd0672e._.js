...
function StyledGroupedBarChart({ financialData = [], amountType = "In Crores" }) {
    ...
    const parsed = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["useMemo"])(()=>{
        ...
        const rows = (Array.isArray(financialData) ? financialData : []).filter((r)=>r && String(r.period_ended).toLowerCase().indexOf("period") === -1);
        // sort chronologically
        rows.sort((a, b)=>sortKey(a.period_ended) - sortKey(b.period_ended));
        return {
            periods: rows.map((r)=>r.period_ended),
            assets: rows.map((r)=>toNum(r.assets)),
            revenue: rows.map((r)=>toNum(r.revenue)),
            net_worth: rows.map((r)=>toNum(r.net_worth)),
            profit_after_tax: rows.map((r)=>toNum(r.profit_after_tax)),
            reserves_and_surpluse: rows.map((r)=>toNum(r.reserves_and_surpluse)),
            total_borrowing: rows.map((r)=>toNum(r.total_borrowing))
        };
    }, [
        financialData
    ]);
...
//# sourceMappingURL=src_components_ipodetailspages_FinancialsChart_jsx_4bd0672e._.js.map